...
func main() {
	...
	// 连接服务器
	err := client.ConnectToServer("127.0.0.1", 8081, 30, 1024)
	if err != nil {
		log.Fatalf("Failed to connect to server: %v", err)
	}

	// 发送测试消息
	testData := []byte("Hello WebSocket Server!")
	...
	// 等待输入
	fmt.Println("WebSocket client started. Press Enter to exit.")
	fmt.Scanln()
